...
EnergyFolks.populateWidget = function(data, id, height, source) {
    ...
    EnergyFolks.$.each(data.data, function(i, v) {
        ...
        output += '<div data-params="' + EnergyFolks.$.param(info.params) + '" class="ef_item' + (first ? ' first' : '') + (v.highlighted ? ' ef_highlight' : '') + '" style="height:' + height + 'px;"><div class="line_one"><h4>' + info.title + '</h4></div><div class="line_two">' + info.widget + '</div></div>';
        first = false;
    });
