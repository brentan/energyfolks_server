...
EnergyFolks.$(function() {
    EnergyFolks.$(window).on('hashchange',function() {
        ...
        if(hash.substr(1,8) == 'comment_') {
            ...
            hash = hash.split('_');
            EnergyFolks.ajax('get_comment', {model: hash[2], id: hash[3]}, function(data) {
                EnergyFolks.$('#ef_add_comment_' + hash[1]).before(EnergyFolks.comment_item(data.data, false));
            });
        }
    });
    EnergyFolks.$('body').on('click','.ef_reply_link', function() {
        if(EnergyFolks.$(this).closest('.ef_comment_replies').length > 0) {
            //subcomment
            var el = EnergyFolks.$(this).closest('.ef_comment_replies').find('.ef_add_comment');
        } else {
            //main comment
            var el = EnergyFolks.$(this).closest('.ef_comment_item').next('.ef_comment_replies').find('.ef_add_comment');
        }
        el.show();
...
EnergyFolks.comment_item = function(c, show_reply) {
    ...
    try {
        var timestring = '';
        if(elapsed < (3600))
            timestring = '' + Math.ceil(elapsed / 60) + ' minutes ago';
        else if(elapsed < (3600*36))
            timestring = '' + Math.ceil(elapsed / 3600) + ' hours ago';
        else if(elapsed < (3600 * 24 * 6))
            timestring = EnergyFolks.date('l', Math.ceil(when/1000));
        else if(elapsed < (3600*24*30))
            timestring = EnergyFolks.date('M j', Math.ceil(when/1000));
        else
            timestring = EnergyFolks.date('M j, Y', Math.ceil(when/1000));
    } catch(err) {
        timestring = 'Unknown';
    }
    output += timestring;
...
EnergyFolks.MD5 = function (string) {
